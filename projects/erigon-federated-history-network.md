...
```
┌────────────────────┐   ┌──────────────┐   ┌─────────────────┐
│ any client's era1  │──►│  era2ss      │──►│  epoch-rounded  │
│ (geth/reth/nimbus) │   │  (D2)        │   │  .seg + .idx    │
└────────────────────┘   └──────────────┘   │  (D1)           │
                                            └────────┬────────┘
                                                     │
                                                     ▼
...
**D2 — `era2ss`**. The reverse of Erigon's shipped `ss2era`. Reads era1 from any client's export, writes Erigon-format `.seg`/`.idx`. Any operator can now contribute to the snapshot swarm without running Erigon.

**D3 — `ss2rpc` (BlockOnly minimal)**. Read-only JSON-RPC served directly from `.seg`/`.idx`, no live DB / EVM / CL. ~12 methods covering block-explorer and subgraph-indexer usage, with `eth_getLogs` as the anchor — the most expensive method on public RPC providers. State-touching methods (`eth_call` etc.) return typed errors and are follow-up scope.
...
```
era1 (8192 blocks) ──parse era1──▶ {headers, bodies, receipts, TD} × 8192
                    ──feed writer──▶ {headers, bodies, txs}.seg + .idx
```
...
Reuse Erigon's existing read-only RPC handlers, wired to a **snapshot-only backend** that reads from `.seg`/`.idx` only. A method whitelist enforces BlockOnly; anything requiring state returns `-32005 missing prestate`.
...
**RPC handler decoupling (D3).** Erigon's RPC handlers assume a live full backend. Wiring them to a snapshot-only backend may require refactoring shared interfaces or extracting a minimal method subset. Actual effort is unknown until W11 orientation.
...
2. `era2ss` binary producing `.seg`/`.idx` byte-equivalent to Erigon's own snapshots, from geth / reth / nimbus era1 exports.
